...
class movieRatings:
    ...
    def __init__(self,root):
        ...
        self.box_value.set(listOfPeople[0])
        self.choosePersonBox = ttk.Combobox(self.profileFrame, textvariable=self.box_value)
        ...
        self.savePersonButton = tk.Button(self.profileFrame)
        self.savePersonButton.pack(side=tk.TOP)
        self.savePersonButton.config(text='Save Users',command=self.savePersonFunction)
        
        self.loadPersonButton = tk.Button(self.profileFrame)
        self.loadPersonButton.pack(side=tk.TOP)
        self.loadPersonButton.config(text='Load Users',command=self.loadPersonFunction)
    ...
    def createMaxSizeTable(self):
        for _ in range(self.maxNumMovies):
            self.addMovieFunction()
        for _ in range(self.maxNumRateCrit):
            self.addRatingFunction()
    ...
    def savePersonFunction(self):
        _filename = 'userList.txt'
        _file2write=open(_filename,'w')
        _file2write.write(str(listOfPeople))
        _file2write.close()
        
    def loadPersonFunction(self):
        _filename = 'userList.txt'
        _file2write=open(_filename,'r')
        global listOfPeople
        listOfPeople = _file2write.read().split(', ')
        for elem in listOfPeople:
            elem.strip("'[]")
            elem = elem.replace("'","")
        _file2write.close()
        self.choosePersonBox.config(values=listOfPeople)
...
#        print(masterEntryArray)
    ...
    def saveFileFunction(self, outputPath=True):
...
#        saveAsFilePath = fd.asksaveasfile(filetypes=[('csv files','.csv')], defaultextension=[('.csv')])
        ...
        _headers = masterValueDF.iloc[0]
        _data = masterValueDF.iloc[1:]
        _data.columns=_headers
        saveFilename = str(self.choosePersonBox.get())+'MovieFile.csv'
        _data.to_csv(saveFilename, index=False)
    ...
    def editExcelFunction(self):
        ...
        _path = self.saveFileFunction(outputPath=True)
        os.system("start EXCEL.EXE "+" "+str(_path))

        
    def loadExcelFunction(self):
        self.createMaxSizeTable()
